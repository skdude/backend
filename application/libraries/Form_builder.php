...
class Form {
	...
	public function uploadImage($image, $path)
    {
        ...
        if(isset($_POST["submit"])) {
            ...
            if($check !== false) {
                $this->CI->system_message->set_error("File is an image - " . $check["mime"]);
            } else {
                $this->CI->system_message->set_error("File is not an image");
                return false;
            }
        }
        // Check if file already exists
        if (file_exists($target_file)) {
            $this->CI->system_message->set_error("Sorry, file already exists.");
            return false;
        }
        ...
        if ($_FILES[$image]["size"] > 500000) {
            ...
            $this->CI->system_message->set_error("Sorry, your file is too large.");
            return false;
        }
        ...
        if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"
            && $imageFileType != "gif" ) {
            $this->CI->system_message->set_error("Sorry, only JPG, JPEG, PNG & GIF files are allowed.");
            return false;
        }


            if (move_uploaded_file($_FILES[$image]["tmp_name"], $target_file)) {
                $this->CI->system_message->set_success("The file ". basename( $_FILES[$image]["name"]). " has been uploaded.");
                return true;

            } else {
                $this->CI->system_message->set_error("Sorry, there was an error uploading your file.");
            }
