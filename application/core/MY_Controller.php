...
class MY_Controller extends MX_Controller {
    ...
    {
        $query = $this->db->query("SELECT ag.name, ar.right FROM `admin_group_rights` agr
                            LEFT JOIN `admin_groups` AS ag
                            ON ag.id = agr.admin_group_id
                            LEFT JOIN admin_rights AS ar
                            ON ar.id = agr.right_id");

        $rights = array();

        foreach ($query->result() as $row)
        {
            $rights[$row->right][] = $row->name;
        }
...
//		$this->mPageAuth = empty($config['page_auth']) ? array() : $config['page_auth'];
	...
	protected function render($view_file, $layout = 'default')
	{
		...
		$this->mViewData['menu'] = $this->menuRights($this->mMenu);
	...
	private function menuRights($menu)
    {
        if (!$this->ion_auth->in_group(array('webmaster', 'admin'))):
            unset($menu['admin_panel']);
        endif;
